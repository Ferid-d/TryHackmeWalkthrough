...
```
There was a login page. I don't have credentials but when we look at the source code, we will get the answer of two questions **(Q3 and Q5) --> (/auth/check and /auth/resetpassword)**. I decided to make a directory scan:  
```bash
...
*PoC* - (Proof of Concept) file is a sample file or code designed to prove that a specific vulnerability in cybersecurity actually exists and can be exploited.  
----
...
```
Yeah as you remember, we have root privilege to run the exiftool. Since you ran it with sudo, your command runs with the highest power (Root), successfully creating a special "Root-door" called rootbash.  
```bash
